<template>
  <div>
    ...
    <el-card style="width: 100%;margin: 10px;" shadow="always">
      ...
      <el-select v-model="innSpec.keySelectType" @change="handleKeySelectTypeChange" style="width: 240px">
        <el-option
      ...
      <el-select  style="width: 100px;margin-left: 20px;" v-model="innSpec.nodeKeyExpressionParam1" @change="handleNodeKeyExpressionParam1" placeholder="向上几级的Key"
        v-show="innSpec.keySelectType==CONST_KEY_SELECT_TYPE.UPPER_NODE">
        <el-option v-for="item in levelItems" :label="'向上'+item.label+'级'" :value="item.value"></el-option>
      </el-select>
      <!-- <el-select  style="width: 100px;margin-left: 20px;" v-model="innSpec.nodeKeyExpressionParam2" @change="specKeyParam1Change" placeholder="匹配的属性名"
        v-show="innSpec.keySelectType==CONST_KEY_SELECT_TYPE.UPPER_NODE"
        >
        <el-option v-for="item in levelItems" :label="item.label" :value="item.value"></el-option>
      </el-select> -->
      <el-input  style="width: 240px"
          v-show="innSpec.keySelectType==CONST_KEY_SELECT_TYPE.UPPER_NODE" v-model="innSpec.nodeKeyExpressionParam2" @input="handleNodeKeyExpressionParam2"
          placeholder="匹配的属性名"
      />
      <el-input  style="width: 240px"
          v-show="innSpec.keySelectType != CONST_KEY_SELECT_TYPE.CURRENT_NODE"
          :disabled="innSpec.keySelectType == CONST_KEY_SELECT_TYPE.ALL_CURRENT_LEVEL_NODE"
    ...
    <el-card style="width: 100%;margin: 10px;" shadow="always">
      ...
      <el-checkbox-group v-model="innSpec.outputKeyOrValueType" :min="1" :max="2" @change="outputKeyOrValueTypeChange">
        <el-checkbox v-for="item in outputKeyOrValueTypeItems" :key="item.value" :label="item.label" :value="item.value">
          {{ item.label }}
        </el-checkbox>
      </el-checkbox-group>
      <!-- <el-radio-group v-model="innSpec.outputKeyOrValueType" @change="outputKeyOrValueTypeChange">
        <el-radio-button v-for="item in outputKeyOrValueTypeItems" :label="item.label" :value="item.value" />
      </el-radio-group> -->
      <!-- <el-select  v-model="innSpec.outputKeyOrValueType"  style="width: 240px;" @change="outputKeyOrValueTypeChange">
        <el-option v-for="item in outputKeyOrValueTypeItems" :label="item.label" :value="item.value"></el-option>
      </el-select> -->

      <!-- <el-select  v-model="innSpec.outputKeys" multiple placeholder="Select" style="width: 640px;padding-top: 7px;" @change="outputKeysChange">
        <el-option v-for="item in levelItems" :label="'向上'+item.label+'级'" :value="item.value"></el-option>
      </el-select> -->
    </el-card>
    ...
    <el-card style="width: 100%;margin: 10px;" shadow="always">
      ...
      <el-card v-show="innSpec.outputKeyOrValueType.indexOf(CONST_OUTPUT_KEY_OR_VALUE_TYPE.KEY)!=-1" style="margin: 10px;">
          ...
          <!-- 输出Key的所有配置 -->
          <el-row v-for="expression,index in innSpec.outputKeysExpressions" >
            ...
            <el-tag style="margin:5px;line-height: 24px;">{{ `$(`+expression.outputKeyExpressionParam1+','+expression.outputKeyExpressionParam2+')' }}</el-tag>
            ...
            <div v-show="expression.outputPathType==CONST_OUTPUT_PATH_TYPE.BY_KEY">
              <el-select  style="width: 100px;margin-left: 20px;" v-model="innSpec.outputPathExpressionParam1" @change="outputPathExpressionParamArrChange(expression,$event)" placeholder="向上几级的Key">
                  <el-option v-for="item in levelItems" :label="item.label" :value="item.value"></el-option>
              </el-select>
              <el-select  style="width: 100px;margin-left: 20px;" v-model="innSpec.outputPathExpressionParam2" @change="outputPathExpressionParamArrChange(expression,$event)" placeholder="匹配的第几段">
                <el-option v-for="item in levelItems" :label="item.label" :value="item.value"></el-option>
              </el-select>
      ...
      <!-- 输出Value的所有配置 -->
      <el-card v-show="innSpec.outputKeyOrValueType.indexOf(CONST_OUTPUT_KEY_OR_VALUE_TYPE.VALUE)!=-1" style="margin: 10px;">
        <el-row v-for="expression,index in innSpec.outputValuesExpressions" >
        ...
        <el-row v-show="false">
          <el-col colspan="12">
            <!-- <el-input
                ...
                placeholder="请输入目标路径"
            /> -->
            <el-select  style="width: 200px;" v-model="innSpec.outputPathType" @change="specValueTypeChange" >
              <el-option v-for="item in outputPathTypeItems" :label="item.label" :value="item.value"></el-option>
            </el-select>

            <!-- #region 当选择的输出是键的时候，需要输入键的表达式 -->
            <el-select  style="width: 100px;margin-left: 20px;" v-model="innSpec.outputPathExpressionParam1" @change="outputPathExpressionParamChange" placeholder="向上几级的Key"
              v-show="innSpec.outputPathType==CONST_OUTPUT_PATH_TYPE.BY_KEY">
              <el-option v-for="item in levelItems" :label="item.label" :value="item.value"></el-option>
            </el-select>
            <el-select  style="width: 100px;margin-left: 20px;" v-model="innSpec.outputPathExpressionParam2" @change="outputPathExpressionParamChange" placeholder="匹配的第几段"
              v-show="innSpec.outputPathType==CONST_OUTPUT_PATH_TYPE.BY_KEY">
              <el-option v-for="item in levelItems" :label="item.label" :value="item.value"></el-option>
            </el-select>
            <!-- #endregion 当选择的输出是键的时候，需要输入键的表达式-->
            <el-input style="width: 840px" placeholder="请输入目标路径" v-model="innSpec.outputPathExpression" @input="handleInputSpecValue">
            </el-input>
          </el-col>
        </el-row>
      </template>
...
import {SpecModel,JsonTreeModel,DropBean, OutputKeyExpression} from "../../utils/model/jsonModel";
